...
4. Наличие [Visual Studio Code](https://code.visualstudio.com/) или иного редактора кода.

### Подключение к системе контроля версий
1. Запустить редактор кода, создать пустой проект в папке, например на диске `C:/projects/polprom77`.
2. В редакторе кода открыть терминал, для этого нажать `Ctrl+~` (Visual Studio Code) или использовать терминал отдельно будь то командная строка Windows или Powershell.
3. Перейти в созданную ранее папку проекта с помощью команды: `cd C:/projects/polprom77`.
4. Подключиться к удаленному репозиторию проекта и склонировать исходный код себе на компьютер (важно скопировать и вставить всю строку с точкой в конце): 
...
Нужно открыть [http://localhost:3000](http://localhost:3000) в браузере, чтобы увидеть сайт. Теперь можно работать!\
При правке файлов браузер будет автоматически обновлять изменения на странице, у - удобство.

Если по какой-то причине в логах терминала сыпятся ошибки и проект не запускается, это означает, что установка проекта не была выполнена корректно, проверьте пункт "Установка проекта" выше.
...
- **pages** - страницы сайта (подробнее как управлять страницами: [Next.js Routing](https://nextjs.org/docs/routing/introduction)).
- **public** - содержит публичные данные, картинки, фавиконку, robot.txt, sitemap.xml.
- **styles** - глобальные стили, сейчас мы не используем их, желательно изменения добавлять только в темы.
- **themes** - все стили для компонентов и страниц, а также темы устанавливаются здесь.

### SEO оптимизация
- **next-seo.config.ts** - в корне проекта отвечает за настройки SEO, здесь же расположены мета данные для страниц.
- **next-sitemap.config.js** - в корне проекта отвечает за настройки генерации Sitemap, генерация происходит автоматически.
- **data** - здесь редактируются контент и мета теги для статей, текста и содержание статьей для сайта.
...
Проверить произошла ли успешно деплой изменений на продакшен в разделе Deployment:\
